...
function App() {
  ...
  useEffect(() => {
    ...
    const handleRecordAction = async (event) => {
      const { action, collectionName, id, record } = event.detail
      if (action === 'delete') {
        if (!window.confirm('Delete this record permanently?')) return
        try { await apiCall(await adminUser.getIdToken(), 'DELETE', { collectionName, id }); setNotice('Record deleted.') } catch (requestError) { setError(requestError.message) }
      }
      if (action === 'edit') {
        const name = window.prompt('Update name', record.name || '')
        if (!name?.trim()) return
        try { await apiCall(await adminUser.getIdToken(), 'PATCH', { entity: collectionName === 'users' ? 'user' : 'academic', collectionName, id, ...record, name: name.trim() }); setNotice('Record updated.') } catch (requestError) { setError(requestError.message) }
      }
    }
    window.addEventListener('admin-record-action', handleRecordAction)
    return () => window.removeEventListener('admin-record-action', handleRecordAction)
  }, [adminUser])
...
function UserList({ users, kind }) { const action = (detail) => window.dispatchEvent(new CustomEvent('admin-record-action', { detail })); return <div className="panel"><PanelHeader title={`Registered ${kind}`} count={`${users.length} records`} />{users.length === 0 ? <p className="empty-state">No {kind} accounts found in Firestore.</p> : <div className="user-list">{users.map((user) => <div className="user-row" key={user.uid}><div><strong>{user.name || user.email}</strong><small>{user.email} · {user.department || 'Department not set'} · {user.branchId || 'Branch not set'}</small></div><div className="row-actions"><span className="status-pill">{kind === 'faculty' ? user.level || 'L1' : user.studentId || 'Student'}</span><button className="icon-btn" title="Edit" onClick={() => action({ action: 'edit', collectionName: 'users', id: user.uid, record: user })}>Edit</button><button className="icon-btn danger-btn" title="Delete" onClick={() => action({ action: 'delete', collectionName: 'users', id: user.uid })}>Delete</button></div></div>)}</div>}</div> }
