...
const emptyStudent = { email: '', password: '', name: '', phone: '', studentId: '', department: '', branchId: '', classId: '', hostel: '', roomNumber: '' }
...
function App() {
  ...
  async function createUser(event, role, form, reset) { event.preventDefault(); try { await apiCall(await admin.getIdToken(), 'POST', { ...form, role }); reset(); setNotice(`${role} account created.`) } catch (requestError) { setError(requestError.message) } }
  ...
  async function mutate(action, collectionName, id, record = {}) {
    ...
    let payload = { collectionName, id }
    if (action === 'edit') { const name = window.prompt('Update name', record.name || ''); if (!name?.trim()) return; payload = { entity: collectionName === 'users' ? 'user' : 'academic', collectionName, id, ...record, name: name.trim() } }
    try { await apiCall(await admin.getIdToken(), action === 'delete' ? 'DELETE' : 'PATCH', payload); setNotice(action === 'delete' ? 'Record deleted.' : 'Record updated.') } catch (requestError) { setError(requestError.message) }
  }
...
function Overview({ incidents, faculty, students }) { return <><section className="stats-grid"><Stat label="Active incidents" value={incidents.filter((item) => ['ringing', 'answered'].includes(item.status)).length} /><Stat label="Faculty" value={faculty.length} /><Stat label="Students" value={students.length} /><Stat label="Total incidents" value={incidents.length} /></section><section className="panel"><PanelHeader title="Live incidents" count={`${incidents.length} records`} /><div className="table-wrap"><table><thead><tr><th>Student</th><th>GPS</th><th>Status</th></tr></thead><tbody>{incidents.map((incident) => <tr key={incident.id}><td>{incident.studentName || 'Unknown'}</td><td>{incident.latitude && incident.longitude ? `${Number(incident.latitude).toFixed(5)}, ${Number(incident.longitude).toFixed(5)}` : 'Waiting for GPS'}</td><td>{incident.status || 'unknown'}</td></tr>)}</tbody></table></div></section></> }
function UserManagement({ kind, users, form, setForm, onSubmit, academic, onMutate }) { const faculty = kind === 'faculty'; const change = (field, value) => setForm((current) => ({ ...current, [field]: value })); return <section className="management-grid"><form className="panel form-panel" onSubmit={onSubmit}><PanelHeader title={`Create ${kind} account`} count="Required fields marked *" /><Field label="Full name *"><input value={form.name} onChange={(event) => change('name', event.target.value)} required /></Field><Field label="Email *"><input type="email" value={form.email} onChange={(event) => change('email', event.target.value)} required /></Field><Field label="Temporary password *"><input type="password" minLength="6" value={form.password} onChange={(event) => change('password', event.target.value)} required /></Field><Field label="Phone"><input value={form.phone} onChange={(event) => change('phone', event.target.value)} /></Field>{faculty ? <Field label="Faculty level"><select value={form.level} onChange={(event) => change('level', event.target.value)}>{['L1', 'L2', 'L3', 'L4', 'L5'].map((level) => <option key={level}>{level}</option>)}</select></Field> : <Field label="Student ID *"><input value={form.studentId} onChange={(event) => change('studentId', event.target.value)} required /></Field>}<Field label="Department"><AcademicSelect value={form.department} onChange={(value) => change('department', value)} items={academic.departments} /></Field><Field label="Branch"><AcademicSelect value={form.branchId} onChange={(value) => change('branchId', value)} items={academic.branches} /></Field><Field label="Class"><AcademicSelect value={form.classId} onChange={(value) => change('classId', value)} items={academic.classes} /></Field>{!faculty && <><Field label="Hostel"><input value={form.hostel} onChange={(event) => change('hostel', event.target.value)} /></Field><Field label="Room number"><input value={form.roomNumber} onChange={(event) => change('roomNumber', event.target.value)} /></Field></>}<button className="primary-btn">Create {kind} account</button></form><RecordList title={`Registered ${kind}`} records={users} kind={kind} onMutate={onMutate} /></section> }
